...
```{r Load csv}
data <- data.table::fread("test4final_hw.csv")
```
...
Exclude `id` and `tempo` columns from the dataset
...
## Import model
```{r}
svm_rbf_fit <- readr::read_rds("svm_rbf.rds")

target <- svm_rbf_fit %>% predict(data_to_model)

final_df <- bind_cols(id = data[, "id"], target) %>%
  select(id, target = .pred)

write_csv(final_df, file = "submission1_2022-05-31.csv")
```
