...
set.seed(123)
to_be_removed <- sample(1:nrow(data), 10)
...
X_pca_mel <- pc$x[, 1:20] %>% as.data.frame()
...
X_pca_mel %>% 
  ...
  scale_colour_gradientn(colours = RColorBrewer::brewer.pal(4, "RdYlBu"))
```
```{r PCA Components vs Tempo}
tempo_scaled <- tempo / max(tempo)
mel_scaled <- X_pca_mel$pc_mel11 / max(X_pca_mel$pc_mel3)

X_pca_mel %>%
  ggplot(aes(tempo_scaled, mel_scaled)) +
...
```{r KPCA Components vs Tempo}
tempo_scaled <- tempo / max(tempo)
freq_scaled <- X_kpca_freq$pc_freq2 / max(X_kpca_freq$pc_freq2)

X_kpca_freq %>%
  ggplot(aes(tempo_scaled, freq_scaled)) +
  geom_point(alpha = 0.5) +
  geom_smooth()
```
...
```{r Convert genre numbers into strings}
X_red[, "genre"] <- X_red[, "genre"] %>% 
  lapply(as.character) %>% 
  unlist()
```

<!-- ```{r One-hot encoding of genre} -->
...
data_to_model <- X[, c(1:7011, 7016:7032, 7034:7040)] %>%
  as.data.frame() %>%
  tibble()
data_to_model$tempo <- tempo
```

# Initial Modelling (for us)
...
```{r Initial Compare tuned parameters}
svm_rbf_res %>%
  collect_metrics() %>%
  filter(.metric == "rmse") %>%
